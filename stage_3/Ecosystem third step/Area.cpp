...
void Area::show_area() const
{
	...
	for (int x = 0; x < (int)x_size; ++x) {
		...
		for (int y = 0; y < (int)y_size; ++y) {
			...
			{
				if (it->second->get_x() == x && it->second->get_y() == y)
				{
					it->second->show();
					obj_found = true;
					break;
...
void Area::print_info() const
{
	...
	printf("Wolves amount: %d, mean health: %.2f, sum health: %.2f\n", wolves_amount, (wolves_health/wolves_amount), wolves_health);
	printf("Hares amount: %d, mean health: %.2f, sum health: %.2f\n", hares_amount, (hares_health/hares_amount), hares_health);
	printf("Cabbages amount: %d, mean health: %.2f, sum health: %.2f\n", cabbages_amount, (cabbages_health/cabbages_amount), cabbages_health);
}

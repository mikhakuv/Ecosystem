...
void test_wolf()
{
	...
	assert(w2.get_health() == w2.max_health);
	int int_result;
	int_result = w1 <=> w2;
	assert(int_result == 0);
	bool_result = w1.move(5.0, 0.0);
...
void test_hare()
{
	...
	assert(h2.get_health() == h2.max_health);
	int int_result;
	int_result = h1 <=> h2;
	assert(int_result == 0);
...
void test_area()
{
	Area a1(100.0, 200.0, 3, 10, 20); //проверка создания области
	...
	a1.make_step(); //проверка шага(того, что он совершается)
	assert(a1.current_step() == 1);
}
